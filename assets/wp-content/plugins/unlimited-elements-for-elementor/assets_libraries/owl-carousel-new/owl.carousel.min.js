...
! function(t, e, i, s) {
    ...
    function a(e, i) {
        this.settings = null, this.options = t.extend({}, a.Defaults, i), this.$element = t(e), this._handlers = {}, this._plugins = {}, this._supress = {}, this._current = null, this._speed = null, this._coordinates = [], this._breakpoint = null, this._width = null, this._items = [], this._clones = [], this._mergers = [], this._widths = [], this._invalidated = {}, this._pipe = [], this._drag = {
            time: null,
            target: null,
            pointer: null,
            stage: {
            ...
            switch (this.settings.paddingType) {
                ...
                case "right":
                    ...
                    break;
                case "both":
                    t = (this.width() / this.settings.items).toFixed(3) - this.settings.margin;
                    break;
                default:
                    t = (this.width() / this.settings.items).toFixed(3) - this.settings.margin
            }
        ...
        }), this)), t.each(a.Plugins, t.proxy((function(t, e) {
            this._plugins[t.charAt(0).toLowerCase() + t.slice(1)] = new e(this)
        }), this)), t.each(a.Workers, t.proxy((function(e, i) {
            this._pipe.push({
                filter: i.filter,
    ...
    a.Defaults = {
        ...
        run: function() {
            this._width = this.$element.width(), r("set total width this._width: " + this._width)
        }
    }, {
        ...
        filter: ["width", "items", "settings"],
        run: function(t) {
            if (!0 === this.settings.debug && (o = !0), r("run!"), 1 == this.settings.items && 1 == this.settings.pixel_gap_fix && (this.settings.item_size_gap = 1, r("set items size gap: 1")), r("settings - stage padding type: " + this.settings.paddingType), r("settings - stage padding: " + this.settings.stagePadding), this.settings.paddingType && "none" != this.settings.paddingType || !this.settings.stagePadding || (this.settings.paddingType = "both", r("set stage padding type - both")), r("Item width: " + (i = this.getItemWidth())), this.settings.stagePadding > i) {
                this.settings.stagePadding = 0;
                var e = this.getItemWidth();
                this.settings.stagePadding = Math.floor(.5 * e);
                var i = this.getItemWidth();
                r("set new stagePadding: " + this.settings.stagePadding + " and item width: " + i)
            }
            for (merge = null, iterator = this._items.length, grid = !this.settings.autoWidth, widths = [], t.items = {
                    merge: !1,
                    width: i
                }; iterator--;) merge = this._mergers[iterator], merge = this.settings.mergeFit && Math.min(merge, this.settings.items) || merge, t.items.merge = merge > 1 || t.items.merge, widths[iterator] = grid ? i * merge : this._items[iterator].width();
            this._widths = widths
        }
    }, {
        filter: ["items", "settings"],
        ...
        run: function() {
            ...
            r("Set coordinates"), r(s), this._coordinates = s
        }
    }, {
        filter: ["width", "items", "settings"],
        run: function() {
            ...
            r("set outer stage css"), r(c), "none" == i ? (this.$stage.css(o), s.width(c)) : "both" == i ? (this.$stage.css(a), s.css(c), r("Set stage width css (both)"), r(a)) : "left" == i ? (this.$stage.css(h), s.css(c), r("Set stage width css (left)"), r(h)) : "right" == i && (this.$stage.css(l), s.css(c), r("Set stage width css (right)"), r(l))
        }
    }, {
        filter: ["width", "items", "settings"],
        run: function(t) {
        ...
        filter: ["width", "position", "items", "settings"],
        run: function() {
            if (this.settings.setActiveClass && this.settings.setActiveClass) {
                var t = !0;
                this.settings.setActiveClassOnMobile ? 1 == this.$stage.children(".active").length && (t = !0) : this.settings.setActiveClassOnMobile || 1 == this.$stage.children(".active").length && (t = !1), 1 == t ? this.$stage.children().eq(this.current()).addClass("uc-active-item").siblings().removeClass("uc-active-item") : 0 == t && this.$stage.children().removeClass("uc-active-item")
            }
        }
        ...
        if (this.enter("initializing"), this.trigger("initialize"), this.$element.toggleClass(this.settings.rtlClass, this.settings.rtl), this.settings.shuffle) {
            var t = this.$element;
        ...
        var e, i, n;
        this.settings.autoWidth && !this.is("pre-loading") && (e = this.$element.find("img"), i = this.settings.nestedItemSelector ? "." + this.settings.nestedItemSelector : s, n = this.$element.children(i).width(), e.length && n <= 0 && this.preloadAutoWidthImages(e));
        ...
        switch (t = t || a.Width.Default) {
            case a.Width.Inner:
            case a.Width.Outer:
                e = this._width, n("calc width for dimention: " + t);
            default:
                switch (this.settings.paddingType) {
                    case "left":
                    case "right":
                        r("calc width with stagePadding: " + this.settings.paddingType), e = this._width - this.settings.stagePadding + this.settings.margin;
                        break;
                    case "both":
                        r("calc width with stagePadding: " + this.settings.paddingType), e = this._width - 2 * this.settings.stagePadding + this.settings.margin;
                        break;
                    default:
                        e = this._width + this.settings.margin, r("calc width without stagePadding for dimention: " + t)
                }
        }
        return r("carousel width (no stagepadding and margins): " + e), e
        ...
        var i = this.speed() > 0;
        if (this.is("animating") && this.onTransitionEnd(), i && (this.enter("animating"), this.trigger("translate")), t.support.transition) {
            r("Go to coordinate: " + e), this.$stage.css({
                transform: "translateX(" + e + "px)",
                transition: this.speed() / 1e3 + "s" + (this.settings.slideTransition ? " " + this.settings.slideTransition : "")
            })
        } else i ? this.$stage.animate({
            left: e + "px"
        ...
        } else e.type === a.Type.State && (this._states.tags[e.name] ? this._states.tags[e.name] = this._states.tags[e.name].concat(e.tags) : this._states.tags[e.name] = e.tags, this._states.tags[e.name] = t.grep(this._states.tags[e.name], t.proxy((function(i, s) {
        ...
        return this.each((function() {
            ...
            n || (n = new a(this, "object" == typeof e && e), s.data("owl.carousel", n), t.each(["next", "prev", "to", "destroy", "refresh", "replace", "add", "remove"], (function(e, i) {
                n.register({
                    type: a.Type.Event,
                    name: i
...
function(t, e, i, s) {
    ...
    n.Defaults = {
        ...
        if (this._controls.$relative = (i.navContainer ? t(i.navContainer) : t("<div>").addClass(i.navContainerClass).appendTo(this.$element)).addClass("disabled"), this._controls.$previous = t("<" + i.navElement + ' value="previous item" title="previous item">').addClass(i.navClass[0]).html(i.navText[0]).prependTo(this._controls.$relative).on("click", t.proxy((function(t) {
                this.prev(i.navSpeed)
            }), this)), this._controls.$next = t("<" + i.navElement + ' value="next item" title="next item">').addClass(i.navClass[1]).html(i.navText[1]).appendTo(this._controls.$relative).on("click", t.proxy((function(t) {
                this.next(i.navSpeed)
            }), this)), i.changeItemOnClick) {
            var s = this._core.$stage.children(),
                n = this._core.clones().length / 2;
            s.on("click", t.proxy((function(e) {
                var s = t(e.currentTarget).index();
                this.to(s - n, i.navSpeed, !0)
            }), this))
        }
        for (e in i.dotsData || (this._templates = [t('<button role="button">').addClass(i.dotClass).append(t("<span>")).prop("outerHTML")]), this._controls.$absolute = (i.dotsContainer ? t(i.dotsContainer) : t("<div>").addClass(i.dotsClass).appendTo(this.$element)).addClass("disabled"), this._controls.$absolute.on("click", "button", t.proxy((function(e) {
                var s = t(e.target).parent().is(this._controls.$absolute) ? t(e.target).index() : t(e.target).parent().index();
                console.log(t(e.target).index()), e.preventDefault(), this.to(s, i.dotsSpeed)
